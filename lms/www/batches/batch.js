frappe.ready(() => {
	...
	$(".btn-add-cood").click((e) => {
		show_cood_modal(e);
	});

	$(".btn-add-mang").click((e) => {
		show_mang_modal(e);
	});

	$(".btn-add-hours-request").click((e) => {
		show_hours_request_modal(e);
	});

	$(".btn-submit-hours").click((e) => {
...
const show_cood_modal = () => {
	let cood_modal = new frappe.ui.Dialog({
...
const show_mang_modal = () => {
	let mang_modal = new frappe.ui.Dialog({
		title: "Add Manager",
		fields: [
			{
				fieldtype: "Link",
...
const remove_student = (e) => {
	frappe.confirm(
		"Are you sure you want to remove this volunteer from the activity?",
		() => {
			frappe.call({
				...
				callback: (data) => {
					frappe.show_alert(
						{
							message: __("Volunteer removed successfully"),
							indicator: "green",
						},
						2000
					);
...
const add_cood = (values) => {
	frappe.call({
		method: "frappe.client.insert",
		args: {
			doc: {
				doctype: "Activity Coordinator",
				coordinator: values.cood,
				parenttype: "LMS Batch",
				parentfield: "coordinators",
				parent: $(".class-details").data("batch"),
			},
		},
		callback(r) {
...
const remove_cood = (e) => {
	frappe.confirm(
		"Are you sure you want to remove this coordinator from the activity?",
		() => {
			frappe.call({
				method: "lms.lms.doctype.lms_batch.lms_batch.remove_cood",
				args: {
					cood: $(e.currentTarget).data("student"),
					activity_name: $(".class-details").data("batch"),
				},
				callback: (data) => {
					frappe.show_alert(
						{
							message: __("Volunteer Coordinator removed successfully"),
							indicator: "green",
						},
						2000
					);
...
const add_mang = (values) => {
	frappe.call({
		method: "frappe.client.insert",
		args: {
			doc: {
				doctype: "Activity Manager",
				manager: values.mang,
				parenttype: "LMS Batch",
				parentfield: "managers",
				parent: $(".class-details").data("batch"),
			},
		},
		callback(r) {
	...
	frappe.call({
		...
		args: {
			doc: {
				...
				user: values.mang,
				allow: "LMS Batch",
				for_value:$(".class-details").data("batch"),
			},
		},
		callback(r) {
			frappe.show_alert(
				{
					message: __("Manager Added"),
					indicator: "green",
				},
				2000
			);
...
const remove_mang = (e) => {
	frappe.confirm(
		"Are you sure you want to remove this manager from the activity?",
		() => {
			frappe.call({
				method: "lms.lms.doctype.lms_batch.lms_batch.remove_mang",
				args: {
					mang: $(e.currentTarget).data("student"),
					activity_name: $(".class-details").data("batch"),
				},
				callback: (data) => {
					frappe.show_alert(
						{
							message: __("Manager removed successfully"),
							indicator: "green",
						},
						2000
					);
...
const show_hours_request_modal = () => {
	let hours_request_modal = new frappe.ui.Dialog({
		title: "Add Hours Request",
		fields: [
			{
				fieldtype: "Data",
				label: __("Title"),
		...
		primary_action_label: __("Add"),
		primary_action(values) {
			add_hours_request(values);
			hours_request_modal.hide();
		},
	});
	hours_request_modal.show();
	setTimeout(() => {
		$(".modal-body").css("min-height", "200px");
	}, 1000);
};

const add_hours_request = (values) => {
	frappe.call({
		method: "frappe.client.insert",
		args: {
			doc: {
				doctype: "Hours Request",
				hours_activity: $(".class-details").data("batch"),
				title: values.title,
		...
		callback(r) {
			frappe.show_alert(
				{
					message: __("Hours Request Added!"),
					indicator: "green",
				},
				2000
			);
...
const show_hours_submit_modal = (e) => {
	let hours_submit_modal = new frappe.ui.Dialog({
		...
		primary_action(values) {
			add_hours_submit(e, values);
			hours_submit_modal.hide();
		},
	});
	hours_submit_modal.show();
	setTimeout(() => {
		/* $(".modal-body").css("min-height", "200px");*/
	}, 1000);
};
		...
		fields: [
			{
				...
				filter_description: " ",
				default: "LMS Assignment", // Set default value
...
const remove_assessment = (e) => {
	frappe.confirm("Are you sure you want to remove this task?", () => {
		frappe.call({
