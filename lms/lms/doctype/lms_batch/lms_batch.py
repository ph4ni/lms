...
class LMSBatch(Document):
	...
	def validate_batch_end_date(self):
		if(self.end_date < self.start_date):
			frappe.throw("Activity end date cannot be before the batch start date")
...
@frappe.whitelist()
def remove_cood(cood, activity_name):
	frappe.only_for("Moderator")
	frappe.db.delete("Activity Coordinator", {"coordinator": cood, "parent": activity_name})
	frappe.db.delete("User Permission", {"user": cood, "allow": "LMS Batch", "for_value":activity_name})

@frappe.whitelist()
def remove_mang(mang, activity_name):
	frappe.only_for("Moderator")
	frappe.db.delete("Activity Manager", {"manager": mang, "parent": activity_name})
	frappe.db.delete("User Permission", {"user": mang, "allow": "LMS Batch", "for_value":activity_name})
