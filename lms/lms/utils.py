...
def has_submitted_hours(hoursr,member=None):
	if not member:
		member = frappe.session.user
	filters = {}
	filters["hours_request"] = hoursr
	filters["volunteer"] = member
	return frappe.db.exists("Hours Submission", filters)
...
def getstudhours(hourssubmit):
	hrs = frappe.db.get_value("Hours Submission",hourssubmit,"validated_hours")
	return hrs

def getnumans(submission):
	numm = frappe.db.get_value("LMS Assignment Submission", submission, "num_answer")
	return numm
...
def get_details(doctype, docname):
	if doctype == "LMS Course":
		...
		details = frappe.db.get_value(
			"LMS Batch",
			docname,
			["name", "title", "paid_batch", "currency", "amount", "extrainfo", "question1", "question2"],
			as_dict=True,
		)
...
@frappe.whitelist()
def get_extra_questions(batch_name):
    details = frappe.get_value("LMS Batch", batch_name, ["extrainfo", "question1", "question2"], as_dict=True)
    return details
...
@frappe.whitelist()
def add_student_to_batch_wop(batchname):
	student = frappe.new_doc("Batch Student")
	student.update(
		{
			"student": frappe.session.user,
...
@frappe.whitelist()
def add_cood_perms_for_volunteer(cood, volunteer):
...
@frappe.whitelist()
def add_mang_perms_for_volunteer(mang, volunteer):
	perm = frappe.new_doc("User Permission")
	perm.update(
		{
			"user": mang,
...
@frappe.whitelist()
def add_volunteer_tomapping(batchname, answer1, answer2):
	volmapp = frappe.new_doc("Volunteer Mapping")
	volmapp.update(
		{
	...
	volmapp.save(ignore_permissions=True)
	return f"/batches/details/{batchname}"
